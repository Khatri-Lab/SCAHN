...
dtf.prop.sex.meta = dtf.prop.sex[pid %in% pids]
...
p.es.summary.sex =
  ggplot(data = datadf.sex.summary, aes(x = celltype, y = pooled_ES)) +
  theme_expresso(
    legend_position = "none", legend_justification = c(0, 0),
    legend_key_width = 0.1, legend_key_height = 0.1,
    legend_key_spacing_y = 0.02, panel_background = backgroundcol, plot_background = "white") +
  theme(
    plot.title = element_text(face = 1, margin = margin(0, 0, 0, 0, "line")),
    panel.grid.major.y = element_blank(),
    axis.title.x = element_text(margin = margin(t = 0, unit = "pt")),
    axis.text.y = element_text(size = textsize, face = "plain", color = textcolor)) +
  geom_hline(yintercept = 0, size = 0.3, alpha = 1, color = "grey67") +

  geom_tile(width = 0.35, height = 0.1, position = position_dodge(0.7), alpha = 1,
            fill = "grey42") +
  geom_errorbar(data = datadf.sex.summary,
                aes(x = celltype, ymin = lower, ymax = upper, group = celltype),
                position = position_dodge(0.7), color = "grey7", width = 0.18, size = 0.2) +
  labs(x = " ", y = "Effect size\nmale vs. female", title = NULL) +
...
mat.NES = as.matrix(gsea.expr.sex[, "NES"])
rownames(mat.NES) = gsea.expr.sex$pathway
mat.size = as.matrix(gsea.expr.sex[, "FDR"])
rownames(mat.size) = gsea.expr.sex$pathway
mat.size = 1 - mat.size
...
p.expr.summary.sex =
  Heatmap(mat.NES, rect_gp = gpar(type = "none"), col = col_fun.NES, na_col = "grey93",
          ...
          cell_fun = function(j, i, x, y, width, height, fill) {
            grid.circle(x = x, y = y, r = 10 * mat.size[i, j] * min(unit.c(width, height)),
                        gp = gpar(fill = col_fun.NES(mat.NES[i, j]), col = NA, alpha = 0.9))
            grid.circle(x = x, y = y, r = 10 * mat.size2[i, j] * min(unit.c(width, height)),
                        gp = gpar(col = "grey7", fill = NA, alpha = 0.9))
          }, heatmap_legend_param = list(
            title = "NES, male vs. female", title_gp = gpar(fontsize = textsize, col = textcolor),
            labels_gp = gpar(fontsize = textsize, col = textcolor),
            legend_height = unit(4, "line"), grid_width = unit(0.1, "line"),
...
res.ota[, p_value := ifelse(p_value < 3e-04, 3e-04, p_value)]
res.ota[, label := ifelse(!inset %in% c("other", "IFN"), gene, NA)]
...
p.otz.ageAdj = ggplot(res.ota[!is.na(p_value)], aes(x = log2_or, y = -log10(p_value))) +
  theme_expresso(
    legend_position = c(0.9,1), legend_justification = c(0,1),
    legend_key_width = 0.1, legend_key_height = 0.1,
    legend_key_spacing_x = 0.04, legend_key_spacing_y = 0.01,
    legend_text_size = textsize, panel_background = backgroundcol,
    plot_background = "white", axis_title_size = textsize) +
  theme(
    plot.title = element_text(face = "plain", margin = margin(0, 0, 0, 0, "line")),
    axis.title.x = element_text(margin = margin(t = 0, unit = "pt"))) +
  labs(x = "Log2 odds ratio of male vs. female", title = " ") +
  geom_point(shape = 16, size = 0.5, alpha = 0.5, aes(color = inset)) +
  geom_text_repel(aes(label = label, color = inset), size = pt2mm(5), alpha = 1,
  ...
  scale_y_continuous(limits = c(0, 3.6), breaks = seq(0, 6, 1)) +
  scale_x_continuous(limits = c(-2.2, 3), breaks = seq(-3, 3, 1)) +
...
p.sex.bar.otz.ageAdj =
  ...
  ggplot2::geom_bar(aes(color = inset, fill = inset), stat = "identity", width = 1,
                    linewidth = 0) +
...
cf_gsea = colorRamp2(c(-3, 0, 3), c("#2A0BD9", "white", "#A60021"))

ht.NES.ota.ageAdj = Heatmap(
  nes_mat, col = cf_gsea, name = "NES", cluster_rows = FALSE,
  ...
  show_column_names = FALSE, cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(sprintf("%.2f", nes_mat[i, j]), x, y, gp = gpar(fontsize = textsize, col = textcolor))
  }, right_annotation = rowAnnotation(
    pval = anno_text(ota.table$pval, gp = gpar(fontsize = textsize, col = textcolor)),
    annotation_name_gp = gpar(fontsize = textsize, col = textcolor), annotation_name_side = "top",
    annotation_name_rot = 0, show_annotation_name = TRUE),
  row_names_gp = gpar(fontsize = textsize, col = textcolor),
  column_names_gp = gpar(fontsize = textsize, col = textcolor), show_heatmap_legend = FALSE)
...
dtf.sexMeta.ageAdj.shared[, genelabel := ifelse(
  inset3 == "yes" & inset != "IFN", gene, NA)]
...
p.sex.PBMC.WB.ageAdj = ggplot(dtf.sexMeta.ageAdj.shared,
  ...
  theme_expresso(
    legend_position = c(0.99,1), legend_justification = c(0,1),
    legend_key_width = 0.1, legend_key_height = 0.1,
  ...
  theme(
    plot.title = element_text(face = "plain", margin = margin(0, 0, 0, 0, "line")),
    legend.spacing.y = unit(0.5, "line"), legend.spacing.x = unit(-0.02, "line"),
    legend.box = "vertical", legend.box.just = "left",
    axis.title.x = element_text(margin = margin(t = 0, unit = "pt"))) +
  labs(x = "Log2 odds ratio of male vs. female in WB",
       y = "Log2 odds ratio of male vs. female in PBMC", title = " ") +
  ...
  geom_text_repel(aes(label = genelabel, color = inset), size = pt2mm(5), alpha = 1,
                  force = 1.5, max.overlaps = 40, force_pull = 1.5, box.padding = 0.02,
                  fontface = "italic", segment.size = 0.03, min.segment.length = 0.3,
                  show.legend = FALSE) +
  scale_x_continuous(breaks = pretty_breaks()) +
  scale_y_continuous(breaks = pretty_breaks()) +
  scale_color_manual(values = col_set, labels = cap_first, breaks = names(col_set)) +
  ...
  guides(
    shape = guide_legend(order = 1, override.aes = list(size = 0.8)),
    color = guide_legend(order = 2, override.aes = list(size = 0.8)), alpha = "none", size = "none")
...
ht.NES.WB.ageAdj = Heatmap(
  nes_mat.WB, col = cf_gsea, name = "NES", cluster_rows = FALSE,
  row_names_side = "left", column_title = NULL, show_column_names = FALSE, show_row_names = FALSE,
  cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(sprintf("%.2f", nes_mat.WB[i, j]), x, y,
    gp = gpar(fontsize = textsize, col = textcolor))
  }, right_annotation = rowAnnotation(
    pval = anno_text(WB.table$pval, gp = gpar(fontsize = textsize, col = textcolor)),
    annotation_name_gp = gpar(fontsize = textsize, col = textcolor), annotation_name_side = "top",
    annotation_name_rot = 0, show_annotation_name = TRUE),
  row_names_gp = gpar(fontsize = textsize, col = textcolor),
  column_names_gp = gpar(fontsize = textsize, col = textcolor), show_heatmap_legend = FALSE)
...
ht.NES.PBMC.ageAdj = Heatmap(
  nes_mat.PBMC, col = cf_gsea, name = "NES", cluster_rows = FALSE,
  row_names_side = "left", row_labels = cap_first(rownames(nes_mat.PBMC)),
  column_title = NULL,
  show_column_names = FALSE, cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(sprintf("%.2f", nes_mat.PBMC[i, j]), x, y,
    gp = gpar(fontsize = textsize, col = textcolor))
  }, right_annotation = rowAnnotation(
    pval = anno_text(PBMC.table$pval, gp = gpar(fontsize = textsize, col = textcolor)),
    annotation_name_gp = gpar(fontsize = textsize, col = textcolor), annotation_name_side = "top",
    annotation_name_rot = 0, show_annotation_name = TRUE),
  row_names_gp = gpar(fontsize = textsize, col = textcolor),
  column_names_gp = gpar(fontsize = textsize, col = textcolor), show_heatmap_legend = FALSE)
...
for (score in vars) {
  ...
  p.fh.hc.list.age[[score]] =
    ...
    theme_expresso(axis_text_size = textsize, legend_text_size = textsize,
                   plot_background = "white", legend_position = "none",
                   legend_justification = c(0, 1), legend_key_width = 0.1,
                   legend_key_height = 0.1, legend_key_spacing_x = 0.01,
                   legend_key_spacing_y = 0.01) +
    labs(title = "", x = "Age", y = NULL) +
    theme(axis.text       = element_text(margin = margin(0, 0, 0, 0, "line")),
          ...
          plot.title      = element_text(margin = margin(0, 0, 0.5, -6, "pt")),
          legend.box = element_blank(), legend.direction = "vertical",
...
for (score in vars) {
  ...
  p.fh.hc.list.bmi[[score]] =
    ...
    geom_smooth(method = "loess", show.legend = FALSE, size = 0.5) +
    theme_expresso(axis_text_size = textsize, legend_text_size = textsize,
                   plot_background = "white", legend_position = c(0.85,1),
                   legend_justification = c(0, 0), legend_key_width = 0.1,
                   legend_key_height = 0.1, legend_key_spacing_x = 0.01,
                   legend_key_spacing_y = 0.01) +
    labs(title = cap_first(plottitle), x = "BMI", y = NULL) +
    theme(axis.text       = element_text(margin = margin(0, 0, 0, 0, "line")),
          axis.title.x    = element_text(margin = margin(0.3, 0, 0, 0, "pt")),
          axis.title.y    = element_text(margin = margin(0, 0.3, 0, 0, "pt")),
          plot.title.position = "plot",
          plot.title      = element_text(margin = margin(0, 0, 0.5, -6, "pt")),
          legend.box = element_blank(), legend.direction = "vertical",
          legend.key.size = unit(1, "lines")) +
    scale_y_continuous(breaks = seq(0, 9, 1)) +
    scale_x_continuous(breaks = seq(0, 100, 20), limits = c(15,60)) +
    scale_color_manual(values = c("F" = "#ffab65", "M" = "#65b9ff")) +
...
text_size = textsize, ring_lwd = 0.6) {
  sz_mat  = 1.2 - fdr_mat
  ...
  function(j, i, x, y, width, height, fill) {
    ...
    grid.circle(x, y, r = sz2_mat[i, j] * r_base,
                gp = gpar(fill = NA, col = "grey17", alpha = 1, lwd = ring_lwd))
    grid.text(sprintf("%.2f", v), x, y, gp = gpar(fontsize = text_size,
                        col = ifelse(abs(v) > 0.15, "white", textcolor)))
  }
}
...
ht_ame = Heatmap(
  ...
  row_names_gp = gpar(fontsize = textsize, col = textcolor), name = "both sexes",
  column_title = " ", column_title_gp = gpar(fontsize = textsize, fontface = "plain",
  fill = "white",
                         col = NA), show_heatmap_legend = FALSE, row_names_side = "left",
  cell_fun          = make_cellfun(mat_ame[1:6, ], mat_ame_fdr[1:6, ], cf_ame,
                                   dot_scale = 0.6, fdr_cutoff = 0.05, text_size = textsize),
  heatmap_legend_param = list(
    title = "AME", title_gp = gpar(fontsize = textsize, col = textcolor),
    labels_gp = gpar(fontsize = textsize, col = textcolor), direction = "vertical",
...
ht_ame.M = Heatmap(
  ...
  column_names_gp   = gpar(fontsize = textsize, col = textcolor),
  row_names_gp = gpar(fontsize = textsize, col = textcolor), name = "male",
  column_title = " ", column_title_gp = gpar(fontsize = textsize, fontface = "plain",
  fill = "white",
                         col = NA), show_heatmap_legend = FALSE,
...
cf_glm = colorRamp2(c(-0.602, 0, 0.602), c("#2A0BD9", "white", "#A60021"))
...
ht_coef_list = lapply(seq_along(scores_z[1:7]), function(k) {
  ...
  Heatmap(
    coef_mat, rect_gp = gpar(col = NA), col = cf_glm, name = ht_name,
    width = unit(0.7, "cm"), cluster_rows = FALSE,
    row_names_side = "left", show_row_names = (k == 1),
    row_labels = cap_first(rownames(coef_mat)),
    column_title = " ", column_title_gp = gpar(fontsize = textsize, col = NA),
    show_column_names = FALSE, cell_fun = function(j, i, x, y, width, height, fill) {
      p  = pval_vec[i]
      bg = if (!is.na(p) && p > 0.05) "white" else fill
      grid.rect(x, y, width, height, gp = gpar(fill = bg, col = NA))
      grid.text(sprintf("%.2f", coef_mat[i, j]), x, y,
      gp = gpar(fontsize = textsize, col = textcolor))
    }, right_annotation = do.call(rowAnnotation, c(
      setNames(list(row_anno_text(
        res$glm$`p value`, width = max_text_width(res$glm$`p value`,
        gp = gpar(fontsize = textsize)),
        gp = gpar(fontsize = textsize, col = textcolor))), anno_name),
        list(annotation_name_gp = gpar(fontsize = textsize, col = textcolor),
           annotation_name_side = "top", annotation_name_rot = 0, show_annotation_name = FALSE))),
    row_names_gp = gpar(fontsize = textsize, col = textcolor), show_heatmap_legend = FALSE)
})
...
ht_ame_list = lapply(seq_along(scores_z[1:7]), function(k) {
  ...
  Heatmap(
    ame_mat, rect_gp = gpar(col = NA), col = cf_glm, name = ht_name,
    width = unit(0.7, "cm"), cluster_rows = FALSE,
    row_names_side = "left", show_row_names = (k == 1),
    ...
    show_column_names = FALSE, cell_fun = function(j, i, x, y, width, height, fill) {
      ...
      bg = if (!is.na(p) && p > 0.05) "white" else fill
      grid.rect(x, y, width, height, gp = gpar(fill = bg, col = NA))
      grid.text(sprintf("%.2f", ame_mat[i, j]), x, y, gp = gpar(fontsize = textsize,
                          col = if (is.na(ame_mat[i, j])) NA else textcolor))
    }, right_annotation = do.call(rowAnnotation, c(
      setNames(list(row_anno_text(
        pval_fmt, width = max_text_width(pval_fmt, gp = gpar(fontsize = textsize)),
        gp = gpar(fontsize = textsize, col = textcolor))), anno_name),
        list(annotation_name_gp = gpar(fontsize = textsize, col = textcolor),
           annotation_name_side = "top", annotation_name_rot = 0, show_annotation_name = FALSE))),
    row_names_gp = gpar(fontsize = textsize, col = textcolor), show_heatmap_legend = FALSE)
...
cairo_pdf(file = paste0(dir.fig, "Fig.s2.pdf"), width = 5.8, height = 4.3)
pushViewport(viewport(layout = grid.layout(nrow = 100, ncol = 110)))

draw_col_legend = function(title, rows) {
  pushViewport(viewport(layout.pos.row = rows, layout.pos.col = 102:110))
  draw(Legend(
    col_fun = cf_glm, at = c(-0.6, 0, 0.6),
    title = title, title_gp = gpar(fontsize = textsize, col = textcolor),
    title_position = "leftcenter-rot", title_gap = unit(0.3, "line"),
    labels_gp = gpar(fontsize = textsize, col = textcolor),
    legend_height = unit(3, "line"), grid_width = unit(0.25, "line"),
    direction = "vertical"), x = unit(0, "npc"), just = "left")
  popViewport()
}

### regression coefficients (top half)
pushViewport(viewport(layout.pos.row = 15:55, layout.pos.col = 4:96))
draw(ht_coef_all, newpage = FALSE)
for (k in seq_along(scores_z[1:7])) {
  sc_display = gsub("_z", "", scores_z[1:7][k])
  sc_display = gsub("immature_mature_ratio", "ratio of\nimmature/mature", sc_display)
  decorate_column_title(paste0("ht_", k), {
    grid.text(cap_first(sc_display), x = unit(0.5, "npc"), y = unit(0.1, "npc"),
              rot = 90, gp = gpar(fontsize = textsize, fontface = "plain",
                                  col = textcolor), just = c("left",
                                                             "center"))
  })
  decorate_annotation(paste0("pval_", k), {
    grid.text("pval", x = unit(0.5, "npc"), y = unit(1, "npc") + unit(0.2, "line"),
              rot = 90, gp = gpar(fontsize = textsize, col = textcolor), just = c("left", "center"))
  })
}
popViewport()
draw_col_legend("Coefficient", 18:38)

### AME (bottom half)
pushViewport(viewport(layout.pos.row = 73:98, layout.pos.col = 4:96))
...
dt.sc = tableGrob(
  t(dt.sc), theme = ttheme_default(
    core = list(
      ...
      bg_params = list(fill = c("white")), padding = unit(c(0.3, 0.3), "line")), colhead = list(
      fg_params = list(fontsize = textsize, fontface = "plain"), padding = unit(c(0.3, 0.3), "line")
    ), rowhead = list(
      fg_params = list(fontsize = textsize, fontface = "plain"), padding = unit(c(0.3, 0.3),
      "line"))))
...
pthreecircle = ggplot(data = circle_data) +
  ...
  geom_text(aes(x = xtext, y = y, label = label), size = pt2mm(textsize), color = textcolor) +
  theme_void() +
...
df.twocircles.0.1 = data.table(
  x = rep(1, 2), y = 1:2, color = c("grey88", "grey17"), labeltext = c("FDR > 0.1", "FDR ≤ 0.1"))

p.twocircles.0.1 = ggplot(df.twocircles.0.1, aes(x = x, y = y, label = labeltext, color = color)) +
  geom_point(shape = 21, fill = "grey88", size = 2) +
  geom_text(angle = 0, size = pt2mm(textsize), hjust = -0.12, color = textcolor) +
  theme(text = element_text(size = textsize, color = textcolor)) +
  scale_color_identity() +
...
p.arrow = ggplot() +
  ggplot2::geom_segment(aes(x = 0.1, xend = 0.9, y = 0.5, yend = 0.5), size = .2,
                        arrow = arrow(ends = "both", length = unit(.4, "line"),
                                      type = "closed")) +
  annotate("text", x = 0.5, y = 0.47, size = pt2mm(textsize),
           label = "Enriched in females       Enriched in males") +
  scale_y_continuous(limits = c(0.45, 0.52)) +
  theme_void()

p.arrow.vertical = ggplot() +
  ggplot2::geom_segment(aes(x = 0.5, xend = 0.5, y = 0.1, yend = 0.9), size = .2,
                        arrow = arrow(ends = "both", length = unit(.4, "line"),
                                      type = "closed")) +
  annotate("text", x = 0.47, y = 0.5, size = pt2mm(textsize), angle = 90,
           label = "Enriched in females       Enriched in males") +
  scale_x_continuous(limits = c(0.45, 0.52)) +
  theme_void()


# -- Figure 5 ------------------------------------------------------------------

cairo_pdf(file = paste0(dir.fig, "Figure5.pdf"), width = 7.08, height = 6)
pushViewport(viewport(layout = grid.layout(nrow = 109, ncol = 307)))

### single cell

print(p.es.summary.sex + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 3:50, layout.pos.col = 2:50))

pushViewport(viewport(layout.pos.row = 55:102, layout.pos.col = 11:53))
draw(p.expr.summary.sex, merge_legend = TRUE, heatmap_legend_side = "right",
     align_heatmap_legend = "heatmap_top", newpage = FALSE, background = "white")
popViewport()

print(p.arrow.vertical,
      vp = viewport(layout.pos.row = 55:102, layout.pos.col = 3:9))


print(p.otz.ageAdj + theme(plot.margin = margin(0.1, 0.1, 0, 0.1, "line")),
      vp = viewport(layout.pos.row = 3:38, layout.pos.col = 56:150))
pushViewport(viewport(layout.pos.row = 39:52, layout.pos.col = 66:128))
...
decorate_annotation("pval", {
  grid.text("pval", x = unit(0.5, "npc"), y = unit(1, "npc") + unit(.2, "line"),
            gp = gpar(fontsize = textsize, col = textcolor), just = "bottom")
})
...
print(p.sex.PBMC.WB.ageAdj + theme(plot.margin = margin(0.1, 0.1, 0, 0.1, "line")),
      vp = viewport(layout.pos.row = 56:91, layout.pos.col = 56:150))
pushViewport(viewport(layout.pos.row = 94:108, layout.pos.col = 56:118))
...
popViewport()
pushViewport(viewport(layout.pos.row = 94:108, layout.pos.col = 114:152))
...
### framingham
print(p.fh.hc.list.age$immature + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 5:21, layout.pos.col = 172:202))
print(p.fh.hc.list.bmi$immature +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -15, "pt"))),
      vp = viewport(layout.pos.row = 5:21, layout.pos.col = 203:233))


print(p.fh.hc.list.age$degranulating + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 5:21, layout.pos.col = 239:269))
print(p.fh.hc.list.bmi$degranulating +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -20, "pt"))),
      vp = viewport(layout.pos.row = 5:21, layout.pos.col = 270:300))


print(p.fh.hc.list.age$antiprotease + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 22:38, layout.pos.col = 172:202))
print(p.fh.hc.list.bmi$antiprotease +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -18, "pt"))),
      vp = viewport(layout.pos.row = 22:38, layout.pos.col = 203:233))


print(p.fh.hc.list.age$mature + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 22:38, layout.pos.col = 239:269))
print(p.fh.hc.list.bmi$mature +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -10, "pt"))),
      vp = viewport(layout.pos.row = 22:38, layout.pos.col = 270:300))


print(p.fh.hc.list.age$total + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 39:55, layout.pos.col = 172:202))
print(p.fh.hc.list.bmi$total +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -6, "pt"))),
      vp = viewport(layout.pos.row = 39:55, layout.pos.col = 203:233))


print(p.fh.hc.list.age$IFN + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 39:55, layout.pos.col = 239:269))
print(p.fh.hc.list.bmi$IFN +
        theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line"),
              plot.title = element_text(margin = margin(0, 0, 1, -6, "pt"))),
      vp = viewport(layout.pos.row = 39:55, layout.pos.col = 270:300))


pushViewport(viewport(layout.pos.row = 61:97, layout.pos.col = 181:304))
ht_opt(heatmap_border = FALSE)

lgd.ame = Legend(
  col_fun = cf_ame, at = seq(-clamp_ame, clamp_ame, length.out = 3),
  title = "AME", title_gp = gpar(fontsize = textsize, col = textcolor),
  title_position = "leftcenter-rot", title_gap = unit(0.3, "line"),
  labels_gp = gpar(fontsize = textsize, col = textcolor),
  legend_height = unit(3, "line"), grid_width = unit(0.25, "line"),
...
draw(ht_ame + ht_ame.F + ht_ame.M, merge_legend = TRUE,
     heatmap_legend_list = list(lgd.ame), heatmap_legend_side = "right",
     align_heatmap_legend = "heatmap_top", newpage = FALSE, background = "white")
ht_opt(RESET = TRUE)

for (nm in c("both sexes", "female", "male")) {
  decorate_column_title(nm, {
    grid.rect(gp = gpar(fill = "white", col = NA))
    grid.text(cap_first(nm), gp = gpar(fontsize = textsize, col = textcolor))
  })
}
upViewport(1)


print(p.twocircles0.1  + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 74:76, layout.pos.col = 34:38))
print(p.twocircles + theme(plot.margin = margin(0.1, 0.1, 0.1, 0.1, "line")),
      vp = viewport(layout.pos.row = 79:81, layout.pos.col = 284:288))

### panel labels and titles
grid.text(x = unit(0.01, "npc"), y = unit(0.98, "npc"), label = "a",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
grid.text(x = unit(0.01, "npc"), y = unit(0.5, "npc"), label = "b",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
grid.text(x = unit(0.2, "npc"), y = unit(0.98, "npc"), label = "c",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
grid.text(x = unit(0.2, "npc"), y = unit(0.5, "npc"), label = "d",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
grid.text(x = unit(0.59, "npc"), y = unit(0.98, "npc"), label = "e",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
grid.text(x = unit(0.64, "npc"), y = unit(0.44, "npc"), label = "f",
          gp = gpar(fontsize = panellabelsize, fontface = "bold", col = textcolor))
