...
# outcomes.
#
# Requires 00_setup.R (packages, figure settings, textsize / textcolor,
...
# -- Row labels: italic gene symbol, plus the CD antigen where one exists ------
...
ha = HeatmapAnnotation(
  ...
  annotation_legend_param = list(
    type = list(title = "Type", title_position = "leftcenter", title_gp = gpar(fontsize = 10),
                labels_gp = gpar(fontsize = 10, col = textcolor),
                legend_gp = gpar(fontsize = 10), direction = "horizontal",
                nrow = 1, legend_width = unit(0.2, "line"), legend_height = unit(.01, "line"),
                grid_width = unit(0.2, "line"), grid_height = unit(.01, "line"))
  ), annotation_name_gp = gpar(fontsize = 10), show_annotation_name = TRUE, gap = unit(.05, "line"),
...
pht =
  Heatmap(mat, col = col_fun, na_col = "grey", cluster_columns = FALSE, cluster_rows = TRUE,
          ...
          column_names_side = "top", column_names_rot = 90, column_names_gp = gpar(fontsize = 10),
          row_names_gp = gpar(fontsize = 10, fontface = "italic"), row_names_side = "right",
          row_names_max_width = unit(30, "line"), column_title = "", column_title_side = "bottom",
          column_title_gp = gpar(fontsize = 10, fontface = "plain"), show_heatmap_legend = TRUE,
          heatmap_legend_param = list(title = "Scaled expression", title_gp = gpar(fontsize = 10),
...
cairo_pdf(file = paste0(dir.fig, "Fig.e1.pdf"), width = 4, height = 5)
pushViewport(viewport(layout = grid.layout(nrow = 90, ncol = 90)))
pushViewport(viewport(layout.pos.row = 6:90, layout.pos.col = 2:90))
draw(pht, merge_legend = FALSE, heatmap_legend_side = "right",
     annotation_legend_side = "bottom", newpage = FALSE)
upViewport(1)

### brackets grouping the immature and mature sorted populations
grid.brackets(0.03, 0.951, 0.41, 0.951, lwd=1, ticks=0.5, h=0.01, type=4, col="grey42")
grid.brackets(0.415, 0.951, 0.54, 0.951, lwd=1, ticks=0.5, h=0.01, type=4, col="grey42")
grid.text(x = unit(0.2, "npc"), y = unit(0.977,"npc"),
          label = "Immature Neu", gp = gpar(fontsize = textsize))
grid.text(x = unit(0.4775, "npc"), y = unit(0.977,"npc"),
          label = "Mature Neu", gp = gpar(fontsize = textsize))
dev.off()


# -- Extended figure 9, local settings and helpers -----------------------------
...
# Put back this arm's text size and the prior values of the three shim names. A
# stale global theme_expresso() would silently restyle every script run after
